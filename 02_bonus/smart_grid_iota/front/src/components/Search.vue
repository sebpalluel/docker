    ...
    <div v-if="!isLoading">
      <section class="section">
        ...
        <div v-else>
          ...
          <hr>
          <search-tx v-if="resultType === 'tx'" :iota="iota" :results="results" :store="store"></search-tx>
          <search-tx v-if="resultType === 'tag'" :iota="iota" :isCollapsed="true" :results="results" :store="store"></search-tx>
          <search-bundle v-if="resultType === 'bundle'" :iota="iota" :hash="hash" :results="results" :store="store"></search-bundle>
          <search-address v-if="resultType === 'address'" :iota="iota" :hash="hash" :results="results" :store="store"></search-address>
        </div>
      </section>
    </div>
    ...
    methods: {
      makeQuery (queryObject) {
        if (queryObject.hashes) {
          return new Promise((resolve, reject) => {
            this.store.iota.link.api.getTransactionsObjects(queryObject.hashes, (err, res) => {
          ...
          return new Promise((resolve, reject) => {
            this.store.iota.link.api.findTransactionObjects(queryObject, (err, res) => {
              this.isLoading = false
              if (err || res.length <= 0 || res[0].hash === '999999999999999999999999999999999999999999999999999999999999999999999999999999999') {
                this.searchFailed = true
                resolve([{error: 'No transactions found'}])
      ...
      makeAnyQuery (hash) {
        return new Promise((resolve, reject) => {
          ...
          if (hash.length <= 27) {
            this.store.iota.link.api.findTransactionObjects({tags: [hash]}, (err, res) => {
              this.isLoading = false
              if (err || res.length <= 0 || res[0].hash === '999999999999999999999999999999999999999999999999999999999999999999999999999999999') {
                this.searchFailed = true
                return resolve([])
            ...
            this.store.iota.link.api.getTransactionsObjects([hash], (err, res) => {
              if (err || res.length <= 0 || res[0].hash === '999999999999999999999999999999999999999999999999999999999999999999999999999999999') {
                this.store.iota.link.api.findTransactionObjects({addresses: [hash]}, (err, res) => {
                  if (err || res.length <= 0 || res[0].hash === '999999999999999999999999999999999999999999999999999999999999999999999999999999999') {
                    this.store.iota.link.api.findTransactionObjects({bundles: [hash]}, (err, res) => {
                      this.isLoading = false
                      if (err || res.length <= 0 || res[0].hash === '999999999999999999999999999999999999999999999999999999999999999999999999999999999') {
                        this.searchFailed = true
                        return resolve([])
