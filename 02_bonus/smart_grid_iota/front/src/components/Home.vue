<template>
  <div v-if="this.store.iota">
    ...
    <router-view :iota="iota" :store="store"></router-view>
...
<script>
  export default {
    ...
    methods: {
      searchNow () { // method to put correct path of page
    ...
    computed: {
      isValidSearch () {
        ...
        switch (this.searchType.toLowerCase()) {
          case 'tx':
            return this.store.iota.link.valid.isHash(this.searchInput)
          case 'bundle':
            return this.store.iota.link.valid.isHash(this.searchInput)
          case 'address':
            return this.store.iota.link.valid.isAddress(this.searchInput)
          case 'tag':
            return this.store.iota.link.valid.isTrytes(this.searchInput) && this.searchInput.length <= 27
          case 'any':
            return this.store.iota.link.valid.isTrytes(this.searchInput) &&
    ...
    mounted () {
      ...
      if (this.store.messages.length)
        console.log('Home address: ', this.store.messages[0].address);
      // console.log('Home address: ', this.store.messages[0].address);
  },
