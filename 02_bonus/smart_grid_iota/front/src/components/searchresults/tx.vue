    ...
    <ul v-for="(tx, index) in results">
      <li>
        <b-panel collapsible :open="!inBundle && !isCollapsed" has-custom-template>
          ...
          <tx-property :tx="tx" :iota="iota" :store="store"></tx-property>
        </b-panel>
      </li>
    </ul>
  </div>
...
<script>
  ...
  export default {
    ...
    name: 'search-tx',
    mixins: [ValueHelper],
    props: ['iota', 'results', 'overallStatus', 'inBundle', 'isCollapsed', 'store'],
    filters: {
      shorten (str) {
      ...
      asyncTxStatus: {
        ...
        get () {
          return new Promise((resolve, reject) => {
            this.store.iota.link.api.getLatestInclusion(this.results.map((a) => a.hash), (err, res) => {
              if (err) {
                return resolve([])
              }
              return resolve(res)
