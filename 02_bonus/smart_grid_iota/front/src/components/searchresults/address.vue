<template>
    <div class="container">
      ...
      <div class="box">
        <search-tx :iota="iota" :results="results" :isCollapsed="true" :store="store"></search-tx>
      </div>
    </div>
</template>
  ...
  export default {
    ...
    computed: {
      ...
      balanceFormatted () {
        ...
        return this.balance ? this.toUnits(this.balance, false, this.store.iota) : 'Calculating...'
      },
      ...
      balances: {
        ...
        get () {
          ...
          return new Promise((resolve, reject) => {
			  console.log('this.store.iota.link.api', this.store.iota.link.api)
            this.store.iota.link.api.getBalances([this.hash], 100, (err, res) => {
              if (err) {
                this.balanceRetrievalError = err
                return resolve([])
              }
				console.log('balance get', res.balances);
